...
class OutflowPlot:
    ...
    def get_subcube(self, vmin=None, vmax=None):
        """
        Extract the subcube defined by the region.
        """
        slabcube = self.get_spectral_slab(vmin, vmax)
        if isinstance(self.reg, list) or isinstance(self.reg, Regions):
            subcube = slabcube.subcube_from_regions(self.reg, minimize=False)
        else:
            subcube = slabcube.subcube_from_regions([self.reg], minimize=False)
        return subcube

    def get_subimage(self, data, ww, unit=None):
        """ 
        ...
        subimage = Projection(value=masked_data, unit=unit, wcs=ww_sl)
    ...
    def get_moment0(self, vmin=None, vmax=None):
        ...
        #subimage = self.get_subimage(vmin, vmax)
        return slab.moment0()

    def plot_moment0(self, moment0, vmin=None, vmax=None, ax=None, **kwargs):
        """ 
        ...
        subimage = self.get_subimage(moment0.value, moment0.wcs, unit=moment0.unit)
        if ax is None:
            ax = plt.subplot(projection=subimage.wcs)
        ax.imshow(subimage.value, **kwargs)
    ...
    def plot_moment0_contours(self, moment0, vmin=None, vmax=None, levels=None, ax=None, nlevels=5, **kwargs):
        """ 
        ...
        #moment0 = self.get_moment0(vmin, vmax)
        subimage = self.get_subimage(moment0.value, moment0.wcs, unit=moment0.unit)
        plot_proj_contours(subimage, levels, ax, nlevels, **kwargs)

    def plot_outflows(self, vcen=0*u.km/u.s, vmin=-10*u.km/u.s, vmax=10*u.km/u.s, levels=None, 
                      ax=None, blue_color='blue', red_color='red', nlevels=3, **kwargs):
        """
        ...
        """
        if ax is None:
            ax = plt.subplot(projection=self.get_moment0(vmin=-10*u.km/u.s, vmax=10*u.km/u.s).wcs)
        # Plot redshifted outflow
        moment0_red = self.get_moment0(vmin=vcen, vmax=vmax)
        moment0_blue = self.get_moment0(vmin=vmin, vmax=vcen)
        self.plot_moment0_contours(moment0_red, vmin=vcen, vmax=vmax, levels=levels, ax=ax, colors=red_color, nlevels=nlevels, **kwargs)
        # Plot blueshifted outflow
        self.plot_moment0_contours(moment0_blue, vmin=vmin, vmax=vcen, levels=levels, ax=ax, colors=blue_color, nlevels=nlevels, **kwargs)
...
def plot_proj_contours(mom0, levels=None, ax=None, nlevels=5, **kwargs):
    """ 
    Plot contours of the moment 0 map of the spectral cube.
...
def percentile_list(data, percentages=[70, 80, 90, 95, 98]):
    return [np.nanpercentile(data, p) for p in percentages]
